...
export async function POST(request: Request) {
  try {
    const { feeRecipient, feeMint } = await request.json();
    
    if (!feeRecipient || !feeMint) {
      return NextResponse.json(
        { success: false, error: '❌ Faltan parámetros: feeRecipient, feeMint' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({ success: true, ata, isToken2022 });

  } catch (error) {
    console.error('[createFeeATA] Error:', error);
    return NextResponse.json(
      { 
        success: false, 
        code: 'INTERNAL_ERROR',
